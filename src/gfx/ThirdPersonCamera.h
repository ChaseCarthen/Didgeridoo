...
class ThirdPersonCamera : public Camera
{
	public:
		...
		ThirdPersonCamera (math::vec3f orbit_point = math::vec3f (0.0f, 0.0f, 0.0f),
						   ...
						   bool continuous_pivot = false) : Camera ()
		{
			m_zoom_time  	 	= 0.0f;
			m_zoom_dir   	 	= 0;
			m_orbiting   	 	= false;
		...
		/** 
		  ...
		  */
		void orbit (const math::vec2i& vector)
		{
		...
		/** 
		  ...
		  */
		void pivot (const math::vec2i &vector)
		{
		...
		virtual void renderDebug ()
		{
			...
			glTranslatef (0.0, 0.0, length (m_position));
	...
	protected:
		...
		void checkOrbit (float dt)
		{
			...
			{
				math::quatf tmp;
				math::Quaternion<float>::fromYawPitchRoll (m_orbit_vector[0] * dt * ROTATE_SPEED,
								  			  			   m_orbit_vector[1] * dt * ROTATE_SPEED,
								  			  			   0.0f,
								  			  			   tmp);
				rotateWorld (tmp);
