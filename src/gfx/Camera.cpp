...
void Camera::apply (void) const 
{
	...
	glLoadMatrixf (m_view_matrix.data ());
...
{
	//math::quatf tmp (angle, m_orientation.getXAxis (), true);
...
void Camera::debugRender (void) 
{
	glDisable(GL_LIGHTING);
	glPushMatrix();

	glMultMatrixf (m_view_matrix.data ());
	glTranslatef (m_position.x (), m_position.y (), m_position.z ());
...
void Camera::setPerspective (float fov, float aspect_ratio, float z_near, float z_far)
{
	...
	glLoadMatrixf (m_projection_matrix.data ());
}

// Get the projection matrix.
math::Matrixf Camera::getProjectionMatrix (void)
{
	return m_projection_matrix;
}

// Get the modelview matrix.
math::Matrixf Camera::getModelViewMatrix (void)
{
