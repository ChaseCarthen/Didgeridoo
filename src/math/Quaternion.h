/*
   ...
   Purpose  : This class represents a Quaternion which is stored as a scalar angle (in radians) of rotation about
   	      an axis in three dimensions (Vector.h).

   Change List:

      - 06/04/2009  - Created (Cody White and Joe Mahsman)
*/
...
class Quaternion
{
	public:
		...
		/** 
		  ...
		  */
		typedef math::Vector <T, 3> QVec;
		typedef math::Matrix<T>     QMatrix;
		...
		/** Initialized constructor.  Set create_rotation to true to turn the new
		  ...
		  */
		Quaternion (T radians, QVec axis, bool create_rotation = false)
		{
			if (!create_rotation)
			{
				m_angle = radians;
		...
		/** 
		  * Operator * (quaternion).
		  * @param rhs Quaternion to multiply into this once.
		  */
		inline Quaternion<T> operator* (const Quaternion &rhs) const
		{
			Quaternion<T> tmp;
			tmp.m_angle = (m_angle * rhs.m_angle) - math::dot (m_axis, rhs.m_axis);
			tmp.m_axis  = math::cross (m_axis, rhs.m_axis) + (rhs.m_axis * m_angle) + (m_axis * rhs.m_angle);
			return tmp;
		}
		...
		inline Quaternion<T> operator/ (const Quaternion &rhs) const
		{
			Quaternion<T> tmp;
			tmp.m_angle = (m_angle * rhs.m_angle) + math::dot (m_axis, rhs.m_axis);
			tmp.m_axis  = math::cross (m_axis * -1, rhs.m_axis) - (rhs.m_axis * m_angle) + (m_axis * rhs.m_angle);
			return tmp;
		}
		...
		/** 
		  ...
		  */
		inline Quaternion<T> conjugate (void) const
		{
			Quaternion<T> tmp (m_angle, m_axis * (T)-1);
			return tmp;
		}
		...
		/** 
		  ...
		  */
		inline Quaternion<T> inverse (void) const
		{
		...
		{
			matrix = QMatrix (1 - 2 * (m_axis[1] * m_axis[1] + m_axis[2] * m_axis[2]),
			                  2 * (m_axis[0] * m_axis[1] - m_axis[2] * m_angle),
					          2 * (m_axis[0] * m_axis[2] + m_axis[1] * m_angle),
					          0, // End of first row
					          2 * (m_axis[0] * m_axis[1] + m_axis[2] * m_angle),
					          1 - 2 * (m_axis[0] * m_axis[0] + m_axis[2] * m_axis[2]),
					          2 * (m_axis[2] * m_axis[1] - m_axis[0] * m_angle),
					          0, // End of second row
				              2 * (m_axis[0] * m_axis[2] - m_axis[1] * m_angle),
					          2 * (m_axis[1] * m_axis[2] + m_axis[0] * m_angle),
					          1 - 2 * (m_axis[0] * m_axis[0] + m_axis[1] * m_axis[1]),
					          0, // End of third row
					          0,
					          0,
					          0,
		...
		inline Quaternion<T> dot (const Quaternion &rhs) const
		{
			...
			return_val += math::dot (m_axis, rhs.m_axis);
			return return_val;
		}
		...
		/**
		  ...
		  */
		inline const QVec & getAxis (void) const
		{
			return m_axis;
		}
		...
		/**
		  ...
		  */
		inline Vector <T, 4> getAxis4 (void) const
		{
		...
		/** 
		  ...
		  */
		inline Vector <T, 3> getXAxis (void) const
		{
			QMatrix R;
			toMatrix (R);
		...
		/** 
		  ...
		  */
		inline Vector <T, 3> getYAxis (void) const
		{
			QMatrix R;
			toMatrix (R);
		...
		/** 
		  ...
		  */
		inline Vector <T, 3> getZAxis (void) const
		{
			QMatrix R;
			toMatrix (R);
		...
		/** 
		  ...
		  */
		inline void fromEuler (T x, T y, T z)
		{
			T cos_x = cos (0.5 * x);
			T cos_y = cos (0.5 * y);
			T cos_z = cos (0.5 * z);
		...
		/** 
		  ...
		  */
		inline Quaternion<T> toAxisAngle (void) const
		{
			T scale = (m_axis[0] * m_axis[0]) + (m_axis[1] * m_axis[1]) + (m_axis[2] * m_axis[2]);
			Quaternion<T> tmp (2 * acos (m_angle), m_axis / scale);
			return tmp;
		}
		...
		{
			Quaternion<T> tmp (0, vec);
			Quaternion<T> q = *this;
			q.setAngle (-q.getAngle ());
		 ...
		 */
		inline Quaternion<T> slerp (const Quaternion<T> &start, const Quaternion<T> &end, T t)
		{
			T array[4];
		...
		inline void fromMatrix (const QMatrix &matrix)
		{
			...
			else
			{
				...
				j = nxt[i];
				k = nxt[j];
				s = sqrt ((matrix(i, i) - (matrix(j, j) + matrix(k, k))) + 1);
        ...
        *********************************************************************************************************/
		...
		/** 
		  ...
		  */
		static inline void fromYawPitchRoll (T yaw, T pitch, T roll, Quaternion<T> &result)
		{
			Quaternion<T> _yaw   (cosf ((-yaw)   * 0.5f), math::vec3f (0.0f, sinf ((-yaw) * 0.5f), 0.0f));
			Quaternion<T> _pitch (cosf ((-pitch) * 0.5f), math::vec3f (sinf ((-pitch) * 0.5f), 0.0f, 0.0f));
			Quaternion<T> _roll  (cosf ((-roll)  * 0.5f), math::vec3f (0.0f, 0.0f, sinf ((-roll) * 0.5f)));
